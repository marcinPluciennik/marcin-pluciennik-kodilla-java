...
public class SudokuGame {
    ...
    public void solvingTheGame() {
        ...
        if (row1.size() == 0 && row2.size() == 0 && row3.size() == 0 && row4.size() == 0 && row5.size() == 0 &&
                row6.size() == 0 && row7.size() == 0 && row8.size() == 0 && row9.size() == 0) {
            System.out.println("======================================================");
            System.out.println("UNEXPECTED ERROR OF THE SYSTEM - The board is empty!!!");
            System.out.println("I'm sending your IP to the russian hackers!!!");
            System.out.println("Play again or go to the hell ...");
            System.out.println("======================================================");

        }

        sudokuBoard.getRowBoard().stream()
                .flatMap(x -> x.getRow().stream())
                .filter(y -> y.getPossibilities().length == 1)
                .findFirst()
                .ifPresent(y -> y.setValue(y.getPossibilities()[0]));


        Random r = new Random();
        sudokuBoard.getRowBoard().stream()
                .flatMap(x -> x.getRow().stream())
                .filter(y -> y.getValue() < 0)
                .filter(y -> y.getPossibilities().length > 1)
                .findFirst()
                .ifPresent(y -> y.setValue(y.getPossibilities()[r.nextInt(y.getPossibilities().length)]));


        List<SudokuElement> listOfElements = sudokuBoard.getRowBoard().stream()
                .flatMap(x -> x.getRow().stream())
                .filter(y -> y.getValue() < 0)
